...
interface SprintMeta { id:string; label:string; shortLabel:string; start:Date; end:Date; monthIdx:number; }
...
function generateSprints(year: number): SprintMeta[] {
  ...
  while (start.getFullYear() === year && num <= 27) {
    ...
    sprints.push({ id:`${year}-S${num}`, label:`Sprint ${num} (${fmt(start)} \u2013 ${fmt(end)})`, shortLabel:`S${num}`, start:new Date(start), end:new Date(end), monthIdx:start.getMonth() });
    start = new Date(end); start.setDate(start.getDate()+1); num++;
  }
  return sprints;
}

const SPRINTS_BY_YEAR: Record<number, SprintMeta[]> = {};
AVAILABLE_YEARS.forEach(y => { SPRINTS_BY_YEAR[y] = generateSprints(y); });
...
function SprintFilterBar({
  ...
  selectedSprint: string; setSelectedSprint:(s:string)=>void;
}) {
  const [sprintOpen, setSprintOpen] = useState(false);
  const sprints = SPRINTS_BY_YEAR[selectedYear] || [];
  const activeSprint = sprints.find(s=>s.id===selectedSprint);
  const activeLabel = selectedSprint==="overall" ? "Overall" : activeSprint?.label ?? "Overall";
    ...
    <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" as const }}>
      ...
      <div style={{ display:"flex", gap:4 }}>
        {AVAILABLE_YEARS.map(y=>(
          <button key={y} onClick={()=>{ setSelectedYear(y); setSelectedSprint("overall"); }} style={{
            padding:"5px 12px", borderRadius:99, border:"1px solid",
            borderColor: selectedYear===y?"#00c8ff":"rgba(100,180,255,0.18)",
            background: selectedYear===y?"rgba(0,200,255,0.13)":"rgba(255,255,255,0.03)",
      ...
      {/* Divider */}
      <div style={{ width:1, height:22, background:"rgba(100,180,255,0.12)" }}/>

      {/* Sprint dropdown */}
      <div style={{ position:"relative" as const }}>
        <button onClick={(e)=>{ e.stopPropagation(); setSprintOpen(o=>!o); }} style={{
          display:"flex", alignItems:"center", gap:8,
          padding:"5px 14px 5px 12px",
          borderRadius:99, border:"1px solid",
          borderColor: sprintOpen||selectedSprint!=="overall"?"#00c8ff":"rgba(100,180,255,0.18)",
          background: sprintOpen||selectedSprint!=="overall"?"rgba(0,200,255,0.1)":"rgba(255,255,255,0.03)",
          color: selectedSprint!=="overall"?"#00c8ff":"rgba(140,190,240,0.7)",
          fontSize:11, fontFamily:"'DM Mono',monospace", fontWeight:700,
          ...
          <span style={{ overflow:"hidden", textOverflow:"ellipsis" }}>
            {activeLabel.length>36?activeLabel.slice(0,36)+"…":activeLabel}
          </span>
          <svg width="10" height="10" viewBox="0 0 10 10" fill="none" style={{ flexShrink:0, transform:sprintOpen?"rotate(180deg)":"rotate(0)", transition:"transform 0.25s" }}>
        ...
        {sprintOpen && (
          <div onClick={e=>e.stopPropagation()} style={{
            ...
            width:290, maxHeight:320, overflowY:"auto" as const,
            background:"rgba(9,18,38,0.98)", border:"1px solid rgba(0,200,255,0.22)",
            ...
            {/* Overall option */}
            {["overall"].map(opt=>(
              <button key={opt} onClick={()=>{ setSelectedSprint("overall"); setSprintOpen(false); }} style={{
                width:"100%", textAlign:"left" as const, padding:"10px 14px",
                background:selectedSprint==="overall"?"rgba(0,200,255,0.1)":"transparent",
                border:"none", borderBottom:"1px solid rgba(100,180,255,0.08)",
                color:selectedSprint==="overall"?"#00c8ff":"rgba(160,210,255,0.75)",
                fontSize:11, fontFamily:"'DM Mono',monospace", fontWeight:700,
                cursor:"pointer", display:"flex", alignItems:"center", gap:8,
              }}>
                {selectedSprint==="overall"&&<span style={{color:"#00c8ff"}}>✓</span>}
                <span>Overall</span>
              </button>
            ))}
            {/* Sprint list */}
            {sprints.map(s=>(
              <button key={s.id} onClick={()=>{ setSelectedSprint(s.id); setSprintOpen(false); }} style={{
                width:"100%", textAlign:"left" as const, padding:"8px 14px",
                background:selectedSprint===s.id?"rgba(0,200,255,0.08)":"transparent",
                border:"none", borderBottom:"1px solid rgba(100,180,255,0.05)",
                color:selectedSprint===s.id?"#00c8ff":"rgba(140,185,230,0.65)",
                fontSize:10, fontFamily:"'DM Mono',monospace",
                cursor:"pointer", display:"flex", alignItems:"center", gap:8,
                transition:"background 0.15s",
              }}
                onMouseEnter={e=>(e.currentTarget.style.background="rgba(0,200,255,0.05)")}
                onMouseLeave={e=>(e.currentTarget.style.background=selectedSprint===s.id?"rgba(0,200,255,0.08)":"transparent")}
              >
                <span style={{ width:12, flexShrink:0 }}>{selectedSprint===s.id?"✓":""}</span>
                <span>{s.label}</span>
                {s.id===CURRENT_SPRINT_ID&&selectedYear===CURRENT_YEAR&&(
                  <span style={{ marginLeft:"auto", fontSize:8, padding:"1px 5px", borderRadius:99, background:"rgba(0,229,160,0.15)", color:"#00e5a0", fontWeight:800 }}>NOW</span>
                )}
              </button>
            ))}
          </div>
...
export default function DashboardPage() {
  ...
  const activeSprint = SPRINTS_BY_YEAR[selectedYear]?.find(s=>s.id===selectedSprint);
  const filterLabel = selectedSprint==="overall"
  ...
  return (
    <div style={{ padding:"20px 0 40px" }}>
      ...
      <div style={{
        ...
        <div>
          <h2 style={{ fontSize:18, fontWeight:800, fontFamily:"'DM Sans',sans-serif", color:"#e8f4ff", margin:0, letterSpacing:"-0.02em" }}>
            Team Dashboard
          </h2>
          <div style={{ fontSize:11, color:"rgba(0,200,255,0.55)", fontFamily:"'DM Mono',monospace", marginTop:4, letterSpacing:"0.05em" }}>
            {filterLabel}
          </div>
        </div>
        <SprintFilterBar
          selectedYear={selectedYear} setSelectedYear={setSelectedYear}
          selectedSprint={selectedSprint} setSelectedSprint={setSelectedSprint}
        />
      </div>

      {/* KPI row */}
      <div style={{ display:"flex", gap:14, flexWrap:"wrap" as const, marginBottom:20, ...anim(0.05) }}>
        <ScoreKPI label="Overall Team Score"  value="8,742" color="#00c8ff" sub={filterLabel} delta={6.2}/>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:20, ...anim(0.11) }} className="dash-2col">
        <SprintComparison/>
        <VelocityChart/>
      </div>

      {/* Contribution doughnut + Stacked column */}
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:20, ...anim(0.14) }} className="dash-2col">
