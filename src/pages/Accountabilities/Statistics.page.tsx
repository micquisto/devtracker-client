...
function ScorePointsDial({
  value,
  color,
  delay = 0,
  size = "default",
  glowFilterId = "statistics-score-points-dial-glow",
  label = "Score Points",
}: {
  value: number | null;
  color: string;
  ...
  const circumference = 2 * Math.PI * radius;
  const dashOffset = circumference - (fillPercent / 100) * circumference;
  const dialWidth = size === "prominent" ? 126 : size === "compact" ? 84 : 102;
  // SVG fontSize is viewBox units; keep on-screen value/percent readable.
  const valueFontSize = size === "prominent" ? 36 : size === "compact" ? 28 : 32;
  const percentFontSize = size === "prominent" ? 16 : size === "compact" ? 13 : 14;
  const labelFontSize = size === "prominent" ? 12 : size === "compact" ? 10 : 11;
  const displayValue =
    value === null || !Number.isFinite(value) ? "—" : formatScorePoints(value);
    ...
    >
      ...
      >
        ...
        <text
          ...
          y="114"
        >
          {value === null ? "" : `${Math.round(fillPercent)}%`}
        </text>
      </svg>
      <span
        style={{
