export { default as DashboardPage } from "./Dashboard.page";
export { default as LoginPage } from "./Auth/Login.page";
export { default as SprintPage } from "./Scrum/Sprint.page";
export { default as StoryPointsPage } from "./Scrum/StoryPoints.page";
export { default as StatisticsPage } from "./Accountabilities/Statistics.page";
export { default as AccountabilitiesPage } from "./Accountabilities/Accountabilities.page";
export { default as TasksListPage } from "./Scrum/TasksList.page";
export { default as PlanningPokerPage } from "./Scrum/PlanningPoker.page";
export { default as TestPage } from "./Test.page";
export { default as AppFlowPage } from "./AppFlow.page";
export { default as PublicSprintScoreboardPage } from "./PublicSprintScoreboard.page";
export { default as PublicStatisticsPage } from "./PublicStatistics.page";
export { default as PublicAccountabilitiesPage } from "./PublicAccountabilities.page";
export { default as RequirementsDataPage } from "./Admin/RequirementsData.page";
export { default as SprintRequirementsPage } from "./Admin/SprintRequirements.page";
export { default as SprintDataPage } from "./Admin/SprintData.page";
export { default as CriteriaGradingSetsPage } from "./Admin/CriteriaGradingSets.page";
export { default as DevVelocityPage } from "./Admin/DevVelocity.page";
export { default as AccessControlListsPage } from "./Admin/AccessControlLists.page";
export { default as ChangePasswordsPage } from "./Admin/ChangePasswords.page";
export { default as SprintStoryPointsCheckPage } from "./Admin/SprintStoryPointsCheck.page";
export { default as BackgroundProcessPage } from "./Admin/BackgroundProcess.page";