...
Deno.serve(async (request) => {
  ...
  const { memberId, password } = (await request.json()) as UpdatePasswordRequest;

  if (!memberId || !password) {
    return jsonResponse({ error: "memberId and password are required." }, 400);
  }
  ...
  if (hasAclRows && !canAccessChangePasswords) {
    return jsonResponse({ error: "You do not have access to change user passwords." }, 403);
  }
