drop function if exists public.replace_story_points_for_sprint(uuid, jsonb, boolean);

create or replace function public.replace_story_points_for_sprint(
  p_sprint_id uuid,
  p_rows jsonb,
  p_update_assigned boolean default true,
  p_update_adhoc boolean default true
)
returns void
language plpgsql
security definer
...
begin
  ...
  select
    ...
    case
      when p_update_adhoc then row_data.adhoc_story_points
      else coalesce(existing.adhoc_story_points, row_data.adhoc_story_points, 0)
    end as adhoc_story_points
...
grant execute on function public.replace_story_points_for_sprint(uuid, jsonb, boolean, boolean)
to authenticated;
